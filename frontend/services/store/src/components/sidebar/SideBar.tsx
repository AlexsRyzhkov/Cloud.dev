    ...
    return (
        <nav className={'bg-[#35B3B5] h-screen flex flex-col items-center py-[10px] gap-[60px]'}>
            ...
            <div className={'flex flex-row w-full text-[#35B3B5] items-center gap-4 py-[15px] px-[20px] bg-white'}>
                <div className={`bg-[${user.color}] flex w-[40px] h-[40px] items-center justify-center rounded-full font-semibold text-[20px] text-white`}>
                    {user.firstname[0]}
                </div>
                <div className={'flex grow justify-between items-center'}>
                    <p className={'capitalize font-semibold'}>{user.firstname}</p>
                    <i className="pi pi-sign-out hover:cursor-pointer" onClick={()=>{}}></i>
                </div>
            </div>
        </nav>
    )
